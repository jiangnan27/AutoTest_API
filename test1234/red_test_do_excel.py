...
lll = MyLog(get_module_name())
log_me4 = lll.get_logger()
...
class DoExcel:
    def __init__(self, filepath, sheet_name):
        ...
        log_me4.info('打开 {} 的 {} 表'.format(self.filepath, self.sheet_name))
    ...
    def read_excel(self, case_order=None):
        ...
        # 这一条会破坏Excel数据的原始性，以后再看
        # 这一条会破坏Excel数据的原始性，以后再看
        # 这一条会破坏Excel数据的原始性，以后再看
        # case_title.append('sheet_name')  # 把表名字加进去，为后来用例模板 写入结果 定位表名 做准备。
        ...
        if isinstance(case_order, str) and case_order.lower() == 'all':  # 获取所有行的数据
            ...
            for row in range(max_row):  # 获取每一行的值
                ...
                if re.sub(r'\s+', "", b_str) != "":
                    ...
                    case_value.clear()
                    log_me4.warning('{0} 表 {1} 整行为空，这一整行不纳入获取结果，请知悉。'.format(self.sheet_name, row+2))
                    continue

        # 获取部分行的数据
        elif isinstance(case_order, list):
            for row2 in case_order:
                ...
                if row2 == max_row:  # 超出了最大行就不再取值了
                    break
                log_me4.info('读取 {} 表 - 第{}行的 - 用例数据'.format(self.sheet_name, row2))
                for col2 in range(max_col):  # 获取一整行的值
                    value = table.cell(row2+1, col2+1).value  # 获取每一个单元格的value
                    # log_me4.info('value：{}'.format(value))
                    # log_me4.info('type_value：{}'.format(type(value)))
                    if type(value) == str:
                        case_value.append(value)
                    elif value is None:
                        case_value.append("")
                ...
                if re.sub(r'\s+', "", b_str) != "":
                    ...
                    log_me4.warning('{0} 表 {1} 整行为空，这一整行不纳入获取结果，请知悉。'.format(self.sheet_name, row2+1))
                    continue
        log_me4.info('读取Excel - 反值：{}'.format(json.dumps(case_data, indent=2, ensure_ascii=False)))
        return case_data
    ...
    def write_excel(self, value, row_n, col_n=None):
        ...
        try:
            if col_n is None:
                col_n = 9
            log_me4.info('打开sheet：{}'.format(self.sheet_name))
            table = self.table
            ...
            log_me4.info('修改{}成功：({},{})={}'.format(os.path.split(self.filepath)[1], row_n, col_n, value))

        except PermissionError:
            log_me4.error('修改{0}失败：请关闭已经打开的 {0}。'.format(os.path.split(self.filepath)[1]))
        except KeyError as e:
            log_me4.error('修改{}失败：没有 {} 表'.format(os.path.split(self.filepath)[1], self.sheet_name))
        # except Exception as e2:
        #     log_me4.error(e2)


if __name__ == '__main__':
    # base_file = r'H:\AutoTest\API\my_api_autotest\test_data\base_case_data\api_case.xlsx'
    base_file = r'H:\AutoTest\API\my_api_autotest\test_data\csv_case_data\login.xlsx'
    case_data = DoExcel(base_file, 'home').read_excel()
